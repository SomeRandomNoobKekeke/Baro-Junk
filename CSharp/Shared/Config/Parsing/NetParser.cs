        ...
        if (!dataType.IsPrimitive)
        {
          ...
          if (encode is not null)
          {
            ...
            catch (Exception e)
            {
              if (Verbose)
              {
                IConfig.Logger.Warning($"-- NetParser couldn't encode [{dataType}] into IWriteMessage because [{e.Message}]");
              }
            }
            return;
          }
          ...
          if (Verbose)
          {
            IConfig.Logger.Warning($"-- NetParser couldn't encode [{dataType}] into IWriteMessage because it doesn't have {ConfigLogger.WrapInColor($"public static void NetEncode(IWriteMessage msg, {dataType} data)", "white")} method");
          }
        }
      }
    ...
    public static object Decode(IReadMessage msg, Type T)
    {
      if (DecodeTable.ContainsKey(T))
      {
        ...
        catch (Exception e)
        {
          if (Verbose)
          {
            IConfig.Logger.Warning($"-- NetParser couldn't decode [{T}] from IReadMessage because [{e.Message}]");
          }
      ...
      else
      {
        ...
        if (Verbose)
        {
          IConfig.Logger.Warning($"-- NetParser couldn't decode [{T}] from IReadMessage because [{T}] doesn't have {ConfigLogger.WrapInColor($"public static {T.Name} NetDecode(IReadMessage msg)", "white")} method");
        }
        return Parser.DefaultFor(T);
      }
    }
