...
{
  /// <summary>
  /// a class to copypaste from
  /// not to be used
  /// </summary>
  public class CsCodeGeneratorExample
  {
    public void Experiment()
    {
      ...
      ClassModel complexNumberClass = new ClassModel(complexNumberText)
      {
        ...
        {
          new Constructor(complexNumberText)
          {
            Parameters = new List<Parameter>()
            {
              new Parameter(BuiltInDataType.Double, "real", "0.0"),
              new Parameter(BuiltInDataType.Double, "imaginary", "0.0"),
            },
      ...
      {
        OutputDirectory = @"C:\Program Files (x86)\Steam\steamapps\common\Barotrauma\LocalMods\Baro Junk\Ignore\Output"
      };
      csGenerator.Files.Add(complexNumberFile);
      csGenerator.CreateFiles();
      Logger.Default.Log(complexNumberFile);

    }

    public CsCodeGeneratorExample() => throw new ExecutionEngineException("you died");
